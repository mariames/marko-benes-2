...
const ResponsiveServiceHeroWithDesc: React.FC<ServiceHeroProps> = ({
  title,
  subtitle,
  description,
  portfolioLink,
  ...
  return (
    <div className="flex flex-col lg:flex-row h-auto lg:h-screen w-full overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex mt-16 md:mt-32 lg:mt-0">
          ...
          {leftImgs.map((item, index) => (
            <div key={index} className="relative mb-4 mx-3">
              {/** <img src={item.img} alt={item.name} className="w-full h-full object-cover rounded-lg" /> */}
                ...
                />
                <div className="absolute bottom-0 left-0 right-0 bg-white/75 text-black font-bold p-2 rounded-b-lg">
...
export default ResponsiveServiceHeroWithDesc;
